...
class HiveToolsProductSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $honey = Storage::disk('local')->get('/beekeepers.json');
        $sizes = Storage::disk('local')->get('/Beekeeper_sizes.json');
        ...
        foreach ($sizes_data as $size_obj) {
            ...
            foreach ($honey_data as $obj) {
                ...
                $fake_image = public_path('seeder_images/' . $obj['product_name_english'] . '.' . 'jpg');
                ...
                })->resizeCanvas(1000, 1000, 'center', false, '#ffffff')->save($image_path .    $obj['product_name_english'] . '.' . 'jpg')
                    ->resize(400, 400, function ($constraint) {
                        $constraint->aspectRatio();
                    })->resizeCanvas(400, 400, 'center', false, '#ffffff')
                    ->save($image_path . 'thumbs/' .  $obj['product_name_english'] . '.' . 'jpg')
                    ->resize(285, 285, function ($constraint) {
                        $constraint->aspectRatio();
                    })->fit(70, 70)
                    ->save($image_path . 'thumbs/70px_' .  $obj['product_name_english'] . '.' . 'jpg');
                ...
                if (!$product) {
                    $productId = Product::insertGetId([
                        ...
                        'product_color_en' => 'green, blue, white, red, black, yellow, pink, purple, gray, orange',
                        'product_color_slo' => 'zelena, modra, bela, rdeča, črna, rumena, roza, vijolična, siva, oranžna',
                        ...
                        'product_thumbnail' =>  $obj['product_name_english'] . '.' . 'jpg'
